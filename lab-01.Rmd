...
**NA Stands for not applicable We are getting this because there's a problem with our data. We have undefined values?? **
...
**na.rm	- a logical evaluating to TRUE or FALSE indicating whether NA values should be stripped before the computation proceeds. we need to do na.rm = true**
...
**If the mean is significantly larger than the median, it often indicates the presence of skewed data, where a few extreme values (outliers) are pulling the mean higher. In this case, it's likely that a few locations had exceptionally high visitor numbers, causing the mean to be higher than the median. The median, being less affected by extreme values, provides a more typical measure of central tendency for the majority of locations. This would suggest that most settings have moderate visitor numbers, but a few settings with very high numbers are inflating the overall mean.**
